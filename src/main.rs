...
use movie::service::MovieService;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let repository = movie::repository::MovieRepositoryImpl::new(conn.clone());
    let interactor = movie::interactor::MovieInteractorImpl::new(repository);
    let service = movie::service::MovieService::new(interactor);
    let movie = MovieServer::new(service);
